...
while ~feof(fp)
  tline=fgetl(fp);
  if ~isempty(strfind(tline, 'f')) &  ~isempty(strfind(tline, 'C'))
  ...
  elseif ~isempty(strfind(tline, 'f')) & ~isempty(strfind(tline, 'M'))
...
for ii = 1:size(MITind_ilist,2)
  ...
  for ff=1:size(MITind_flist,2)
    ...
    if ITtype==0
%      MITind_mix=[find(FE_f(:,2)==MITF_f+1);find(FE_f(:,2)==MITF_f-1);intersect(find(FE_f(:,2)==MITF_f),find(FE_f(:,1)~=MITind_f))];% not include d0
      MITind_mix=[find(FE_f(:,2)==MITF_f+1);find(FE_f(:,2)==MITF_f-1);find(FE_f(:,2)==MITF_f)]; % include d0
      MITind_mix(:,2)=MITind_mix(:,1);
    elseif ITtype==1
%      MITind_mix=[intersect([find(FE_f(:,3)==MITF_f)],find(FE_f(:,5)~=FE_f(MITind_f,5)))];%not include d0
      MITind_mix=[find(FE_f(:,3)==MITF_f)];%include d0
      MITind_mix(:,2)=FE_f(MITind_mix,5);
    elseif ITtype==2
%      MITind_mix=[intersect([find(FE_f(:,3)==MITF_f+1);find(FE_f(:,3)==MITF_f-1);find(FE_f(:,3)==MITF_f)],find(FE_f(:,5)~=FE_f(MITind_f,5)))];%not include d0
    ...
    for MITM_f=MITF_fmax:-1:MITF_fmin %for
      ...
      for i=1:size(MITind_mix,1)
        ...
        MC=EM_f(intersect(find(EM_f(:,1)==MITind_f),find(EM_f(:,3)==2*MITM_f)),MITind_mix(i,1)+3); % check Mixing coeffcients
        for k=1:size(TM_L_temp,1)
          ...
          if ~isempty(TM_L) & ~isempty(TM_V) & TM_V~=0
            ...
            for MITM_i=MITF_imax:-1:MITF_imin
              ...
              sum_mL_EM(MITind_M)=sum_mL_EM(MITind_M)+MC*mtrans(ITtype,I,MITJ_i,MITJ_mix(i,1),MITF_i,MITJ_mix(i,2),MITM_i,MITM_f,TM_L,EMtype,MITM_i-MITM_f);
          ...
          elseif ~isempty(TM_L) & ~isempty(TM_V) & TM_V==0
            ...
            for MITM_i=MITF_imax:-1:MITF_imin
              ...
              sum_mL_MM(MITind_M)=sum_mL_MM(MITind_M)+MC*mtrans(ITtype,I,MITJ_i,MITJ_mix(i,1),MITF_i,MITJ_mix(i,2),MITM_i,MITM_f,TM_L,EMtype,MITM_i-MITM_f);
              iMi=iMi+1;
            end
          else
            for MITM_i=MITF_imax:-1:MITF_imin
      ...
      if Ttype==0 %if Ttype
        ...
        for MITM_i=MITF_imax:-1:MITF_imin
          ...
          if ITtype==1
            AM_M(MITind_M,13)=2.1420E10/(2*MITF_f+1)*abs((E_f-E_i))^3*sum_mL_EM(MITind_M)^2+7.5926E-1/(2*MITF_f+1)*abs((E_f-E_i))^5*sum_mL_MM(MITind_M)^2;
          else
            AM_M(MITind_M,13)=2.1420E10*abs((E_f-E_i))^3*sum_mL_EM(MITind_M)^2+7.5926E-1*abs((E_f-E_i))^5*sum_mL_MM(MITind_M)^2;
          end
      ...
      elseif Ttype==1
         ...
         for MITM_i=MITF_imax:-1:MITF_imin
           ...
           AM_M(MITind_M,12)=FE_i(MITind_i,5);
           if ITtype==1
             AM_M(MITind_M,13)=5.7032E4/(2*MITF_f+1)*abs((E_f-E_i))^5*sum_mL_EM(MITind_M)^2+2.8516E5/(2*MITF_f+1)*abs((E_f-E_i))^3*sum_mL_MM(MITind_M)^2;
           else
             AM_M(MITind_M,13)=5.7032E4*abs((E_f-E_i))^5*sum_mL_EM(MITind_M)^2+2.8516E5*abs((E_f-E_i))^3*sum_mL_MM(MITind_M)^2;
           end
           AM_M(MITind_M,14)=abs(AM_M(MITind_M,5)-AM_M(MITind_M,11))*219474.63068;
           iMi=iMi+1;
           E_i=FE_i(find(FE_i(:,1)==MITind_i),4); % Lower level energy   %used for AM
    ...
    if ~isempty(AM)
      iMi=1;
      for k=1:size(AM,1)
        %print to <name>.trans file Check here!!!for the three transitions type
        if ITtype==0
          iMi=1;
        ...
        elseif ITtype == 2
          ...
          for MITM_i=MITF_imax:-1:MITF_imin 
            ...
            Ekays=abs((AM_M(MITind_M,5)-AM_M(MITind_M,4)))*219474.63068;
            fprintf(fpM,'%2.0f\t%3.1f\t%3.1f\t%3.1f\t%16.9f\t%2.0f\t%2.0f\t%3.1f\t%3.1f\t%3.1f\t%16.9f\t%2.0f\t%5.4E\t%16.9f',AM_M(MITind_M,1:14));
            iMi=iMi+1;
            fprintf(fpM,'\n');
          end
        end % end ITtype
