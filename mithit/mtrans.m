...
%                                                                                                %
 ...
 function transm = mtrans(ITtype,I,j1,j2,f1,f2,m1,m2,TM,EMtype,qn)                                
...
elseif ITtype==2  %mit-hfs
  if EMtype==1
    transm=transm+(-1)^(f1-m1+j2+I+f1+1)*sqrt((2*f1+1)*(2*f2+1))*wig3j(f1,1,f2,-m1,qn,m2)*wig6j(j1,f1,I,f2,j2,1)*TM;
  elseif EMtype==2 
    transm=transm+(-1)^(f1-m1+j2+I+f1+2)*sqrt((2*f1+1)*(2*f2+1))*wig3j(f1,2,f2,-m1,qn,m2)*wig6j(j1,f1,I,f2,j2,2)*TM;
  end
end
